...
void print(BinaryTreeNode<int> *root, int k, vector<int> path)
{
    if(root == NULL)
    {
        return;
    ...
    print(root -> left, k - root -> data, path);
    print(root -> right, k - root -> data, path);
}

void PrintPath(BinaryTreeNode<int>* root, int k)
{
    vector<int> path;
    print(root, k , path);
}